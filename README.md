...
Two interactive digital-twin pages for the **EV-CH-007** prototype skateboard
chassis, built with procedural Three.js (r160), ES modules and a zero-build
Node static server.

| Page | What it does |
| --- | --- |
| **Twin** (Structural) | STRAIN / STRESS / DEFORMATION / LOAD / ANOMALY modes on the chassis, region heat-map behaviour and a structural decision panel. |
...
Five assessment modes on the same chassis:
- **STRAIN** — measured strain channels vs baseline regions.
- **STRESS** — derived stress contour behaviour.
- **DEFORMATION** — deflection exaggeration visualisation.
- **LOAD** — load-case emphasis.
- **ANOMALY** — flags anomalous regions from the sensor evidence (SG02
  excluded).

The right-hand **Structural Assessment** panel maps sensor evidence to
regions (B1–B4 / F1 / C1 / R1) and gives a decision output.
...
- **Left nav**: Twin · Hardware Twin · Manufacturing · Experiments · Analytics ·
  Passport · Alerts — *Twin* and *Hardware Twin* are clearly distinct pages.
...
| --- | --- | --- |
| 1 | 3D chassis fills ~65–75 % of screen | `createView` camera/home; `analyze.js` reports center density 0.94–0.97 across all 18 frames |
...
| 5 | Layer + zone filters fade (not remove) | `applyFilters()`; verified in `verify.js` + screenshots 19/20 |
...
| 9 | Renders at 1920×1080, no console errors | `render.js` (18 shots in `shots/`) — all frames `vehicle-ok`, zero browser errors |

## QA scripts

| Script | Purpose |
| --- | --- |
| `node render.js` | Headless Chrome at 1920×1080 → `shots/` (18 screenshots covering both twins + all HW modes) |
| `node analyze.js` | Pixel-metric review of every shot (vehicle density, inspector/strip occupancy) |
| `node verify.js` | 30+ DOM/runtime assertions (summary, inspector, filters, twin modes, SG02 health) |
...
js/core/scene.js        renderer, camera, OrbitControls, flyTo, frame hooks
js/core/chassis.js      procedural skateboard chassis + region meshes
